<template>
  <div class="my-12 pl-12 pr-12 lg:pr-20">
    <h2 class="text-2xl border-b-2 font-bold">Services</h2>

    <div class="flex-col lg:grid grid-cols-2 grid-rows-2 gap-y-16 gap-x-8 px-8 py-12">
        <div class="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 my-12 lg:my-0">
            <h2>Web Development</h2>
            <p> Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ducimus, enim aliquid! Architecto nesciunt doloremque
                 tempore labore aspernatur
                 consequatur reiciendis rerum at laborum dolor. Consequatur, obcaecati quisquam saepe vitae suscipit cupiditate! 
            </p>
        </div>

        <div class="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 my-12 lg:my-0">
            <h2>Data Analysis</h2>
            <p>
                Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ducimus, enim aliquid! Architecto nesciunt doloremque
                 tempore labore aspernatur
                 consequatur reiciendis rerum at laborum dolor. Consequatur, obcaecati quisquam saepe vitae suscipit cupiditate!
            </p>

        </div>

        <div class="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 my-12 lg:my-0">
            <h2>Machine Learning</h2>
            <p>
                Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ducimus, enim aliquid! Architecto nesciunt doloremque
                 tempore labore aspernatur
                 consequatur reiciendis rerum at laborum dolor. Consequatur, obcaecati quisquam saepe vitae suscipit cupiditate!
            </p>
        </div>

        <div class="transition ease-in-out delay-150 hover:-translate-y-1 hover:scale-105 my-12 lg:my-0">
            <h2>Critical Thinking</h2>
            <p>
                Lorem ipsum dolor sit, amet consectetur adipisicing elit. Ducimus, enim aliquid! Architecto nesciunt doloremque
                 tempore labore aspernatur
                 consequatur reiciendis rerum at laborum dolor. Consequatur, obcaecati quisquam saepe vitae suscipit cupiditate!
            </p>
        </div>
    </div>
  </div>
</template>

<script>
export default {
    name: 'ServicesComp'
}
</script>

<style>

</style>